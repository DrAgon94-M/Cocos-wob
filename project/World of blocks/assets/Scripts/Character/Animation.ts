...
export class Animation{
    ...
    constructor(node : Node){
        this._node = node
        this._uiTransform = node.getComponent(UITransform) as UITransform;
    }

    private get _eulerY(){
        return this._node.eulerAngles.y;
    }

    private get _standEuler(){
        return new Vec3(0, this._eulerY, 0);
    }
    ...
    playIdle(){
        if (this._node.eulerAngles != this._standEuler){
            this._setRotationTo(this._standEuler);
        }
    ...
    playMove(speedScale : number){
        ...
        if (speedScale != this._curMoveScale){
            ...
            this._setRotationTo(new Vec3(0, this._eulerY, -angle));
        }

        this._tween("move" + speedScale.toString())
            ?.to(0.5, {eulerAngles : new Vec3(0, this._eulerY, -angle + 2)}, {easing : "cubicOut"})
            .to(0.5, {eulerAngles : new Vec3(0, this._eulerY, -angle)}, {easing : "linear"})
            .union()
            .repeatForever()
            .start()
    }
                ...
                ?.to(0.1, {scale : this._doubleJumpStartScale}, {easing : "linear", onComplete : () =>
                    {   
                        ...
                        this._tween("doubleJumpRotate")
                            ?.to(0.25, {eulerAngles : new Vec3(0, this._eulerY, -180)}, {easing : "linear", onComplete : () =>{
                                this._node.eulerAngles = this._standEuler;                               
                            }})
                            .repeat(2)
                            .union()
                            .start();
    ...
    private _tween(source : string){
        ...
        else
        {
            ...
            this._curTween?.stop();
            this._curTween = tween(this._node);
    ...
    private _setAnchor(x : number, y : number){
        ...
        this._node.position = new Vec3(pos.x, pos.y + diff * this._originSize.y, pos.z)
